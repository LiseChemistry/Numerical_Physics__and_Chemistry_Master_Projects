...
const double temperature = 1; // imposé dans l'exo kT = 0.01
const int nombre_cycles =4000; //1 cycle = 100 tentatives, réussies ou non
...
int main() 
{
    ...
    initialiser_Configuration_Cristalline(tab_particules, L); 
    ...
    FILE *pos_file = fopen("positions_data_1kT_L40.txt", "w");
    FILE *energie_file = fopen("energie_data_10_L40_equi.txt", "w"); 
    FILE *pression_file = fopen("pression_data.txt","w"); 

    int tentative = 0;
    int tentative_acceptee=0; 
    int index_modidi=0; 
    int taille_bloc=1; //Moyenne toutes les 10 configs
